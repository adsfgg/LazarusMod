...
function TunnelExit:SetVariant(tunnelVariant)
...
TunnelExit.kTunnelInfestationRadius = 7
function TunnelExit:GetInfestationRadius()
    return TunnelExit.kTunnelInfestationRadius
end
...
if Server then
    ...
    function TunnelExit:CheckForClogs()
    ...
    function TunnelExit:OnKill(attacker, doer, point, direction)
        ...
        local tunnel = Shared.GetEntity(self.tunnelId)
        if tunnel then
            tunnel.timeCollapseStart = Shared.GetTime()
...
function TunnelExit:OnConstruct(builder)
    if Server then
...
if Server then
    function TunnelExit:OnTag(tagName)
        ...
        if tagName == "eat_clog_start" then
            self:AddTimedCallback(TunnelExit.EatAClog, kClogEatTime)
